...
def Show_Maps_Elements(MAP_title,MAP_text,MAP_boton,cnt_container,upcnt_color,Select_Folder_MAP,Ventana_Info_MAP,Function_Maps,resultado_label_MAP):
    
    # ---- Titulo de la pestaña:
    if MAP_title["tlt_tlt_MAP"] is None:
        MAP_title["tlt_tlt_MAP"] = wnfun_lib.Label_Image('/Maps_title.png', 1100, 65, cnt_container,"white",0.495,0.13)

    #---- Descripcion introductoria:
    if MAP_text["txt_cnt_MAP"] is None:
        MAP_text["txt_cnt_MAP"] = wnfun_lib.Label_Image('/Maps_text.png', 532, 345, cnt_container,"white",0.30  ,0.55)
        
    # ---- Seleccionar carpeta:
    if MAP_boton["btn_slc_MAP"] is None:
        MAP_boton["btn_slc_MAP"] = wnfun_lib.Button_Image('/Select_FolderV2.png', 278, 65, cnt_container,"white",0.78,0.5,Select_Folder_MAP) 
    
    # ---- Informacion:
    if MAP_boton["btn_inf_MAP"] is None:
        MAP_boton["btn_inf_MAP"] = wnfun_lib.Button_Image('/Info.png', 27, 27, cnt_container,"white",0.885,0.468,Ventana_Info_MAP)
        
    # ---- Generar:
    if MAP_boton["btn_clb_MAP"] is None:
...
def Hide_Maps_Elements(title_MAP,MAP_title,text_MAP,MAP_text,boton_MAP,MAP_boton):
...
#%% ====== FUNCION PERDIDAS ===================================================
def Function_MAPS_Elements(carpeta_seleccionada_DNO):
    ...
    # manzana y taxonomia, junto con las carpetas del modelo y los shapes.
        ...
        if rutas_hdf5 == []:
            ...
            tk.messagebox.showinfo("ERROR", warning)
        elif len(rutas_hdf5) < 4:
            warning = "En la carpeta no estan todos los archivos .hdf5 que se requieren"
            tk.messagebox.showinfo("ERROR", warning)
        elif len(rutas_hdf5) > 4:
            warning = "Hay más de cuatro archivos .hdf5 en la carpeta."
            tk.messagebox.showinfo("ERROR", warning)
        else:
        ...
        if CalcMode_PRD_mnz == "Probabilistico" and CalcMode_PRD_scc == "Probabilistico" and CalcMode_DNO_mnz == "Probabilistico"  and CalcMode_DNO_scc == "Probabilistico":
            calculation_mode = "Probabilistico"
        elif CalcMode_PRD_mnz == "Deterministico" and CalcMode_PRD_scc == "Deterministico" and CalcMode_DNO_mnz == "Deterministico" and CalcMode_DNO_scc == "Deterministico":
