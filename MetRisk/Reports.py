...
def Show_REP_Elements(FCH_title,FCH_text,FCH_boton,cnt_container,upcnt_color):
    
    # ---- Titulo de la pestaña:
    if FCH_title["tlt_tlt_FCH"] is None:
        FCH_title["tlt_tlt_FCH"] = wnfun_lib.Label_Image('/Reportes_title.png', 1100, 65, cnt_container,"white",0.495,0.13)

    #---- Descripcion introductoria:
    if FCH_text["txt_cnt_FCH"] is None:
        FCH_text["txt_cnt_FCH"] = wnfun_lib.Label_Image('/Reportes_text.png', 1000, 94, cnt_container,"white",0.50,0.27)
        
    # ---- Seleccionar carpeta:
    if FCH_boton["btn_slc_FCH"] is None:
        FCH_boton["btn_slc_FCH"] = wnfun_lib.Label_Image('/Reportes_municipios.png', 900, 380, cnt_container,"white",0.5,0.62) 
...
def Function_FCH_Elements(carpeta_seleccionada_DNO):
    ...
    # manzana y taxonomia, junto con las carpetas del modelo y los shapes.
        ...
        if calculation_mode == "Probabilistico":
            ...
            plt.savefig(os.path.join(os.getcwd(), "Diagrama_Circular.jpg"), dpi=300, bbox_inches='tight', pad_inches=0)
            plt.show()
            ...
            wnfun_lib.mapa_gen_Infograph(0,area_shp,manzana_shp,mapdata_collapsed,'collapsed',colormap_personalizado,'Map_Collapsed.jpg',0.5,2,6.5,'Edificios colapsados')
            ...
            pdf_base_path = os.path.join(os.path.join(os.getcwd(),"css"), "Ficha_tecnica.pdf")
            ...
            c.setFont("Aptos_Bold", 10)
            c.setFillColorRGB(51/255, 86/255, 107/255) #------------------------------------ Color #33566B
            c.drawString(104, 164, f"{int(fatalities_val):,}") #---------------------------- fallecidos

            c.setFont("Aptos_Bold", 10)
            c.setFillColorRGB(51/255, 86/255, 107/255) #------------------------------------ Color #33566B
            c.drawString(158, 164, f"{np.around(fatalities_PRC,1):.2}") #------------------- fallecidos prc

            c.setFont("Aptos_Bold", 10)
            c.setFillColorRGB(51/255, 86/255, 107/255) #------------------------------------ Color #33566B
            c.drawString(285, 164, f"{int(injured_val):,}") #------------------------------- heridos

            c.setFont("Aptos_Bold", 10)
            c.setFillColorRGB(51/255, 86/255, 107/255) #------------------------------------ Color #33566B
            c.drawString(90, 152, f"{np.around(injured_PRC,1):.2}") #----------------------- heridos prc
    ...
    return output_pdf_path,calculation_mode
